...
# Task 17.3: Auto Review Fix ワークフロー改善 + 動作確認

## Background

17.1 (実装) と 17.2 (設定) は完了済み。動作確認で以下の問題が判明し、ワークフロー再設計が必要。

### 判明した問題

| # | 問題 | 原因 | 対処状況 |
|---|------|------|---------|
| 1 | OIDC token エラー | `github_token` 未設定 | PR #35 で修正済み (develop マージ済み) |
| 2 | `fatal: not in a git directory` | `actions/checkout` 未設定 | PR #37 で修正済み (develop マージ済み) |
| 3 | `error_max_turns` | `--max-turns 5` が小さすぎ | PR #38 で削除 → 再設計必要 |
| 4 | 再レビューで `COMMENTED` になる | Code Rabbit の仕様 | `commented` 条件追加 (PR #39 マージ済み) |
| 5 | `commented` の walkthrough 誤発火 | 条件が広すぎる | **未対処 → 今回対処** |
| 6 | Claude が長時間走りコスト超過 | セーフガード不足 | **未対処 → 今回対処** |
| 7 | 並列実行で重複修正のリスク | concurrency 未設定 | **未対処 → 今回対処** |

## Plan

### Step 1: ワークフロー再設計・実装

`.github/workflows/fix-review.yml` を以下の設計で書き換える:

- [ ] `concurrency` グループ追加 (`autofix-${{ github.event.pull_request.number }}`)
- [ ] インラインコメント確認ステップ追加 (GitHub API で review comments 数を取得)
  - `gh api repos/{owner}/{repo}/pulls/{pr}/reviews/{review_id}/comments --jq 'length'`
  - 0 なら skip (walkthrough のみ)、> 0 なら続行
- [ ] `timeout-minutes: 10`
- [ ] `claude_args: "--max-turns 10"` (公式デフォルト)
- [ ] `commented` 条件追加 (changes_requested || commented)
- [ ] 既存ステップの条件を `check-actionable` と連鎖させる

コスト制御 5 層:
```
Layer 1: インラインコメント > 0     → walkthrough 誤発火防止
Layer 2: --max-turns 10             → Claude ターン数上限
Layer 3: timeout-minutes: 10        → ジョブ時間上限
Layer 4: ループカウント上限 3 回     → 繰り返し修正上限
Layer 5: concurrency group          → 同一 PR 並列実行防止
```

### Step 2: 仕様ドキュメント更新

- [ ] `docs/auto-review-fix.md` を改善版設計に同期更新
  - トリガー条件、Safety Layers、Workflow Example を更新

### Step 3: develop へマージ

- [ ] ブランチ作成・プッシュ・PR 作成
- [ ] develop へマージ (pull_request_review はベースブランチのワークフローを参照するため)

### Step 4: テスト用 PR で E2E 動作確認

- [ ] テスト用ブランチ作成 (意図的な問題ファイル付き)
- [ ] PR 作成 → Code Rabbit レビュー待ち
- [ ] ワークフロー発火確認
- [ ] Claude Code Action がコード修正・コミットプッシュすることを確認
- [ ] ループカウントが PR body に記録されることを確認

### Step 5: クリーンアップ

- [ ] テスト PR クローズ
- [ ] テストブランチ削除
- [ ] `docs/tasks.md` の 17.3 チェックボックス更新
- [ ] `tasks/lessons.md` に最終レッスン記録
